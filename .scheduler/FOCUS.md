...
- **2026-07-24 16:25 (via `scheduler -i`):** Cosmetic bug in cmd_glance's NEXT UP keyword extraction (found 2026-07-24 while fixing the same parser's date-prefix/numbered-list undercounting): for any bullet in cmd_idea's own `scheduler -i` insertion format (`- **$ts (via `scheduler -i`):** $text`), the bold span is ONLY the timestamp+source tag -- the real content sits unbolded after it. cmd_glance's keyword extraction takes everything between the first '**' pair as the keyword, so a freshly-dropped scheduler -i idea shows as NEXT UP: '1/N: 2026-07-20 19:59 (via scheduler -i)' instead of the actual idea text, until that project's next run retitles/reformats it into real scope. Confirmed live on home-assistant's row after the date-prefix fix un-hid its one real backlog item. Not a correctness bug (the count is right, the item is real) -- just a bad keyword for a bullet in this specific shape. Fix: when the extracted bold-span keyword itself looks like a bare timestamp (matches /^[0-9]{4}-[0-9]{2}-[0-9]{2}/), fall back to the first ~28 chars of unbolded text right after the bold span instead.
