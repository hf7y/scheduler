...
PROPOSAL 1 -- **DONE 2026-07-27, paced cycle.** the autocommit watcher must refuse a file it cannot safely adopt. Built in `bin/scheduler`'s `cmd_commit_file` (the single shared implementation `scheduler -i` and the vim auto-commit hook both call via `scheduler _commit-file` -- one fix, both callers covered, nothing outside this repo touched): before the existing diff/staleness checks, refuses (leaves the tree dirty, message to stderr, exit 1) if the file has a conflict marker at line start (`^<<<<<<<`, `^=======$`, `^>>>>>>>`) or a duplicate `^## <heading>` line -- fence-skipped with the same awk convention `bin/blockers-freshness-check.sh` already uses, so a `## realisateur` shown inside a fenced code example doesn't false-positive. Verified offline against three fabricated scratch-repo scenarios reproducing the live exhibit's shapes: a conflict-marker block (refused, exit 1, file left dirty), a duplicate `## realisateur` heading (refused, exit 1, file left dirty), and a `## realisateur` heading legitimately repeated inside a fenced block (NOT refused, committed normally) -- plus `bash -n` on `bin/scheduler`. Proposal 2 (machine-append anchoring) and proposal 3 (blockers-freshness-check.sh's own reader-side UNKNOWN-on-zero-sections guard) are separate, already-shipped or still-open guards on the same failure; this is only the writer-side refusal.

Live exhibit (historical, for the record): 0e9b6a6, "Human edit via scheduler: BLOCKERS.md (2026-07-27T01:15)", 106 insertions and zero deletions. The ~:30 watcher caught BLOCKERS.md in the middle of a live vimdiff merge and committed it under Zach's name. What it adopted: two unresolved conflict blocks (<<<<<<< /tmp/vzTGDTh/5 ... >>>>>>>) fencing two of Zach's own answers, and a verbatim duplicate of the whole ## gardien / ## senechal / ## realisateur tail -- an answered copy plus a blank-slot copy.
