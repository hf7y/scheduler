...
- **2026-07-29 01:49 (via `scheduler -i`): SCHEDULER'S HALF DONE (2026-07-29 paced cycle, dexter) -- the 13-project fan-out is NOT, see the DONE note at the end of this entry.** QUESTIONS.md CONTRACT AMENDMENT -- the fourth unregulated interface (human->system instruction). Filed by realisateur /ideate 2026-07-29, Zach-directed, four AskUserQuestion decisions. Owner: scheduler (canonical text lives in examples/QUESTIONS.md.template and embedded in bin/scheduler).
...
DONE 2026-07-29 (paced cycle, on dexter) -- SCHEDULER'S HALF ONLY, `f74b9c1` + `7d3ba04`. Rules A-D and the one-directional channel rule now live in `examples/QUESTIONS.md.template` (canonical, full text), in the header `bin/scheduler`'s `cmd_ask` writes into a QUESTIONS.md it creates (compact form -- the copy a brand-new project actually gets, which would otherwise never see the amendment), and in `examples/nightly-batch.md.template`'s answer-processing step (where the rules BIND at run time; rules that live only in a template no runner reads are decoration). `DESIGN-NOTES.md` records the amendment and its reason. Verified by `tests/questions-contract-witness.sh` (new, 27 assertions, auto-discovered by `tests/run-all.sh`), which checks the generated header by REALLY RUNNING `scheduler ask` against a throwaway `SCHED_ROOT` rather than grepping the source; negative control run (deleting rule D drops the suite to 26/1, so it is not passing vacuously); full suite 8/8 green under `env -u SSH_AUTH_SOCK`. One thing the rollout surfaced and fixed on the way: the rule bullets are `- **A. ...**`-shaped, which `bin/questions-lint.sh` reads as ENTRIES -- a generated file with ONE real question reported 4 findings. Fixed in `7d3ba04` (pre-`## ` preamble is header prose), with a deliberate fail-open guard so a headingless legacy file is still linted from line 1.

STILL OPEN from this entry, unchanged: the 13 already-distributed project copies (realisateur, senechal, secretaire, bibliothecaire, ecosim, abletim, quatre-vingt-douze, realisateur-research-ecosim, gardien, vim-arcade, nine-speakers, groc-mangr, sequestria) carry the un-amended text. That fan-out writes into OTHER REPOS, which an unattended paced cycle is forbidden to do, so it cannot be delivered from this job regardless of effort -- it needs `check-project-busy <project>` per target and a session that is not this one. Also untouched: this repo's own `.claude/commands/nightly-batch.md`, which unattended runs are hard-refused on writing (same constraint already recorded against the `publish-report.sh` wiring above).
...
plus 'gh auth setup-git' on mandark (dexter already had the credential helper; mandark did not). Filed to senechal per the machine-wide-config protocol (senechal FOCUS.md a1a8b1e). Revert on either host: git config --global --unset url.https://github.com/.insteadOf

VERIFICATION, the exact bar this ecosystem keeps failing: probed the EXACT 15-URL set that commit 3a45bf3 introduced, from BOTH hosts, AFTER the change. 15/15 READY on mandark, 15/15 READY on dexter, 30/30 total. Then observed live: bibliothecaire's brief push at 23:32 went out over HTTPS through the rewrite (404ea3b..fa21cd0), i.e. confirmed at the point of real use and not only in a probe.

WHY THE REVERSAL. ecosim's BRIEF-dexter-migration.md refuted the deploy-key plan on a mechanism nobody had raised: SSH offers candidate keys SERIALLY against GitHub's MaxAuthTries limit of 6, so accumulating ~19 per-repo keys on one host eventually breaks authentication INTERMITTENTLY rather than visibly. That is the failure signature this ecosystem is worst at catching. Recorded as acted-upon-but-unsourced and filed to bibliothecaire for primary-source verification (briefs/sensor-variety-REQUEST.md, secondary section) -- it decided a real change and should not stay a claim in a commit message.

WHERE ecosim'S BRIEF WAS RIGHT AND THIS SESSION WAS WRONG: it found that dexter ALREADY held a working account-wide GitHub credential over HTTPS via gh auth setup-git. This session probed only the SSH form and concluded 'dexter has no generic GitHub identity', which was too broad -- it has one, over a transport that was not tested. A negative result on one transport was reported as a negative result on the host.

WHERE THIS SESSION CORRECTS THE BRIEF: the brief tested dexter only, and its one-line fix is correct FOR DEXTER. mandark had NO https credential helper at all (gh git_protocol=ssh, no credential.* config, and 'git ls-remote https://github.com/hf7y/ecosim.git' failed there). Applying the insteadOf line alone, as the brief proposed, would have fixed dexter and left mandark broken -- and mandark is the host actually dispatching all 15 of those projects today, since nothing has moved yet. That is why the revert of 3a45bf3 (356ecb0) was still the right call in the moment, and why the fix needed 'gh auth setup-git' on mandark as well as the rewrite. The revert and the rewrite are COMPLEMENTARY, not competing: the revert kept mandark running tonight; the rewrite unblocks both hosts going forward.

STILL QUEUED, NOT BUILT -- Zach chose 'both' on the blocker-#6 fork: SEPARATE THE FIELDS ANYWAY. Keep REPO_URL as the cloneable remote for the executing host; add a distinct GITHUB_REPO="owner/repo" that gh_repo_slug() reads, with slug-derivation-from-REPO_URL demoted to a fallback. The credential fix makes 3a45bf3's URLs work, so blocker #6 could now be re-closed by simply re-applying it -- but the underlying coupling (one field serving both 'what GitHub repo is this' and 'what can this host clone') is what produced tonight's outage and would produce the next one. crt and abletim are the standing proof the two facts differ: both legitimately use ssh://mandark-lan/... reachable only from dexter, and neither has any business carrying a GitHub URL just so the issues feature can parse a slug.

IF 3a45bf3 IS RE-APPLIED: it must NOT restore schedule/scheduler.conf as a regular file. That file was a SYMLINK to ../.scheduler/schedule.conf -- the single-source reference for the per-project folder model every other project is migrating onto -- and 3a45bf3 replaced it with a full copy (mode 120000 -> 100644), creating two configs that can silently drift. The revert restored the symlink. Worth a guard regardless of what happens to blocker #6: flag any tracked symlink under schedule/ that becomes a regular file.

STATE AT FILING: dexter unstalled (was 2 behind with a dirty _paced.dexter.conf blocking every PULL; Zach's uncommitted policy-reconciliation edit committed as 11340e9, wtul un-parked as 891586b, both pushed to main). dexter's rotation now reads 'slots=4 :: crt crt crt wtul' -- a two-project sample rather than the one-project sample the brief flagged. wtul dropped from _paced.conf on mandark in the same paired change (e135d18), at weight 1 rather than the 2 its dexter line carried, preserving Zach's same-day cadence cut.
...
READINESS MATRIX — `git ls-remote` run ON DEXTER against every project's REPO_URL from schedule/*.conf. Result: 2 of 19 reachable.
  READY:   chezz, wtul
  BLOCKED: 15 projects, ALL with the identical error `git@github.com: Permission denied (publickey)` — dexter has no generic GitHub identity, only four per-repo aliases in ~/.ssh/config (github-scheduler-deploy, github-wtul-deploy, github-chezz-deploy, github-gardien-deploy) plus mandark-lan and potato.
  BLOCKED (own cause): home-assistant — `ssh: Could not resolve hostname github-ha-deploy` (alias absent on dexter). gardien — REPO_URL is still `/home/zach/git-remotes/gardien.git`, a mandark-local path that does not exist on dexter.

So commit 3a45bf3 ("REPO_URL: point all 15 projects to GitHub") is a live instance of the BUILD-DISCIPLINE row "verified where the consumer reads it": correct on mandark, never tested from the host that now needs it. The migration is not 17 independent problems, it is one credential decision plus two named outliers.

METHOD NOTE, recorded deliberately as dogfooding data: the FIRST run of that probe reported all 19 READY. The loop piped `git ls-remote` through `head -1`, so `$?` was head's status, always 0. A migration-readiness gate that cannot fail is worse than none — it would have greenlit 17 moves that all fail at first clone. This is the exact "no exit-0 no-ops; pipefail+SIGPIPE guarded" row, hit while building the very check meant to enforce readiness. Whatever readiness gate gets built MUST have a negative test (assert a known-unreachable remote reports BLOCKED), not just a positive one.

STALE CLAIM RETIRED: schedule/_paced.dexter.conf's wtul block says step 3 is "[BLOCKED -- HUMAN, needs repo admin] ... `gh` is not installed on dexter at all (`gh: command not found`, checked 2026-07-25), so there is no authenticated path to `gh repo deploy-key add` from here." FALSE as of 2026-07-28: `gh` is at /usr/bin/gh on dexter and `gh auth status` reports logged in as hf7y with token scopes 'admin:public_key', 'gist', 'read:org', 'repo'. Deploy-key provisioning is therefore SCRIPTABLE from dexter with no GitHub-UI step. wtul's own step 4 (`git ls-remote` live-verified FROM dexter) now PASSES — see matrix above — so wtul is un-parkable per its own checklist, though that flip is deliberately NOT done here.

NO FREEZE MECHANISM EXISTS. grep for PAUSE/MAINTENANCE/freeze/halt across bin/scheduler, bin/usage-paced-runner.sh, lib/*.sh returns nothing. Stopping the ecosystem today means hand-editing THREE crontabs: mandark/zach (weight-audit 06:30, paced-runner */5, sweep */15), dexter/zach (paced-runner */5 only — no sweep tick, no weight-audit, no USAGE_CEILING override), and mandark/svc-vaporwave (aedile 03:00, vkv-inventory 04:00 — fixed cron, entirely outside the paced system).

MANDARK CAN DRIVE DEXTER NON-INTERACTIVELY: `ssh -o BatchMode=yes dexter` succeeds. This retires a constraint asserted twice in this repo's own docs as if dexter were reachable only in person — _paced.dexter.conf's "must be amended in a human session ON dexter" and FOCUS.md:1309 "A session running ON dexter edits the header itself". Both were written against an assumption that is no longer (or was never) true.

DEXTER IS DIVERGENT RIGHT NOW: ~/scheduler is at 65fdf19, 2 commits behind origin/main, with an UNCOMMITTED modification to schedule/_paced.dexter.conf (23 lines of superseded-policy comment deleted, plus a new line `# Zach note. Scheduler may move to dexter eventually 260728`). Human intent sitting unversioned in the file that governs dexter's dispatch. Must be committed or deliberately discarded BEFORE any orchestrated migration — a migration that begins against a dirty divergent checkout fails the "deploy verified against a git ref" row on its first step.

WRAPPER TRANSLATION IS THE PER-PROJECT UNIT: every mandark participant line points at ~/.local/bin/<proj>-nightly-batch-loop.sh. dexter's ~/.local/bin contains NO project wrappers — only claude, node, npm, npx, crt-brain-shell, usage-gate.sh, usage-paced-runner.sh. Each move is therefore a REWRITE into `bin/scheduler-run <proj> batch` form (the crt/wtul precedent), not a line copy, and each rewrite has its own failure mode (does the conf set BATCH_PROMPT? does it set BATCH_SCRIPT, which scheduler-run ignores entirely?).

=== THE FOUR DECISIONS (Zach, interactive, 2026-07-28) ===

1. FREEZE = A GIT-TRACKED FREEZE FILE. Build a tracked file (e.g. schedule/_freeze) that usage-paced-runner.sh, the sweep tick, and scheduler-run all check and REFUSE LOUDLY on. One commit stops both hosts, because git is already the channel between them. Explicitly NOT chosen: hand-commenting crontabs (machine-wide config on three identities, and the un-comment step is the reversal-risk shape), and setting every enabled=0 (does not stop the sweep tick or svc-vaporwave, so it is not a freeze).
   OPEN, and it is the honest gap in this choice: a git-tracked file does NOT reach svc-vaporwave's two fixed-cron jobs (aedile 03:00, vkv-inventory 04:00). Those run under a different user, on fixed cron, outside the paced runner. A freeze that silently misses two live jobs while reporting success is worse than one that refuses to claim completeness. Either extend the check into those wrappers or have the freeze STATE what it does not cover.
   REQUIRED: loud refusal, never a silent skip. And the freeze must be checked at DISPATCH time by each consumer, not merely honored by the runner — otherwise a directly-invoked wrapper bypasses it.

2. ORCHESTRATION = INSTALL SCHEDULER ON DEXTER; SCHEDULER BUILDS DEXTER'S SCHEDULE VIA GIT; TWO AGENTS DOGFOOD. Zach's own formulation, not one of the three offered options, and it is a stronger shape than any of them. Reading it out: scheduler is not merely PRESENT on dexter, it is the ENGINE there; dexter's schedule is GENERATED by scheduler from git-tracked config rather than hand-maintained; and two agents (one per host) run the migration as a live dogfooding exercise.
   THE CONSEQUENCE THIS IMPLIES, flagged as inference rather than asserted as decided: if scheduler BUILDS dexter's schedule, then `schedule/_paced.dexter.conf` and `schedule/_paced.conf` stop being hand-written files and become GENERATED ARTIFACTS, with host assignment becoming a field on each project's own conf (a HOST= line) — one source of truth, exactly the "config read from one source, not retyped per file" row. That would retire the two-writer invariant ("dexter writes ONLY this file, mandark writes ONLY _paced.conf") not by violating it but by DISSOLVING it: nobody hand-writes either file, so two writers cannot conflict. It would also retire, structurally, the doctrine-decay problem this repo has now hit twice (a policy header asserting a rule the ecosystem has reversed) — generated files carry no hand-written rationale to go stale.
   NEEDS ZACH'S CONFIRMATION before anyone builds toward it: is the generated-schedule / HOST-field reading what you meant, or did you mean something narrower (scheduler installed and current on dexter, but _paced.dexter.conf still hand-maintained there)? The two lead to very different builds and this session did not assume.

3. CREDENTIALS = PER-REPO DEPLOY KEYS, ONLY FOR PROJECTS ACTUALLY MOVING NOW. Keeps the minimal-scope precedent (dexter_mandark_deploy, dexter_wtul_deploy, chezz) rather than granting dexter one account-wide key that could reach every repo Zach owns. Scoped to the move list rather than all 19, since 7 of 19 are enabled=0 and would be paying for keys they never use.
   NOW CHEAP, per the gh finding above: this is a scriptable loop on dexter — ssh-keygen per project, `gh repo deploy-key add` with write access, add the Host alias to ~/.ssh/config, then `git ls-remote` LIVE-VERIFY before the participant line moves. That last step is the crt bar and is non-negotiable; wtul's 2026-07-25 revert exists precisely because a move landed ahead of its credential.
   MOVE LIST NOT YET FIXED — needs stating explicitly rather than inferred. Currently live on mandark: chezz, chezz-sweep, home-assistant, wtul, realisateur, gardien, senechal, quatre-vingt-douze, bibliothecaire, ecosim. scheduler is deferred by decision 4. crt already runs on dexter. aedile + vkv-inventory are svc-vaporwave fixed-cron and are a separate question entirely. The remaining 7 are parked.

4. SCHEDULER ITSELF = DECIDE AFTER THE OTHERS LAND. Deliberate last step: migrate everything else, gather the dogfooding data, then decide with evidence whether two-host coordination was worth having at all. This supersedes nothing — _paced.dexter.conf's "scheduler is deliberately NOT here" note and its open divergence question both stand until that decision is taken. Zach's uncommitted dexter note ("Scheduler may move to dexter eventually 260728") is consistent with deferral, not with moving now.

=== WHY ZACH WANTS THIS ORCHESTRATED THROUGH SCHEDULER ITSELF ===
Stated purpose, in his words: this is dogfooding, and "very important data for a more intelligent scheduler philosophy that's not simply about fair round robin turns."
What would actually produce that data: the migration units are HETEROGENEOUS in readiness — some blocked on a credential, some on a wrapper rewrite, some on nothing at all. Round-robin gives a blocked unit exactly as many turns as a ready one, and the blocked unit burns a gate probe and a failing clone on each. That is not hypothetical here: it is precisely what _paced.dexter.conf's own wtul block warned about ("burns a gate probe + a failing clone every rotation slot"), and what the 2026-07-25 revert was for.
So the migration is a natural experiment in READINESS-GATED dispatch: a participant that cannot proceed should YIELD its turn rather than consume it. Recording per-unit state transitions (blocked -> ready -> moved -> verified), with the reason for each block and the wall-clock spent in each state, is the dataset. That is the thing to instrument BEFORE the migration runs, because it cannot be reconstructed afterward.

=== SEQUENCING (proposed, not decided) ===
 0. Resolve dexter's dirty/divergent checkout — commit or discard Zach's _paced.dexter.conf edit, fast-forward to origin/main. Nothing else is safe until the two hosts agree on what the config says.
 1. Build the freeze file + loud refusal (decision 1), including an explicit statement of what it does not cover (svc-vaporwave). Test the negative case.
 2. Confirm the decision-2 reading (generated schedule + HOST field, or narrower) with Zach. Build follows the answer, not the inference.
 3. Fix the two named outliers: gardien's REPO_URL (still a mandark-local path; a github-gardien-deploy alias already exists on dexter, so repointing the conf may be the whole fix), and home-assistant's missing alias.
 4. Fix the readiness probe properly, with a negative test, and run it as the gate — not as a report.
 5. Per-project, in readiness order: provision key -> live-verify ls-remote FROM dexter -> translate wrapper to scheduler-run -> paired participant edit (drop on mandark, add on dexter, same change) -> verify one real run.
 6. Only then revisit scheduler's own move (decision 4).

=== THE OTHER MIGRATION-BLOCKING DEFECTS ALREADY FILED, which this makes urgent rather than theoretical ===
- 8c94eff / the 14:55 entry: the sweep path COMMITS AND DOES NOT PUSH. Confirmed in two repos, one of them with a purely local bare remote, so the ssh-agent hypothesis is refuted and the cause is in the sweep path itself. Under "move everything to dexter" every project inherits this, because a dedicated clone resetting --hard to origin cannot see a commit that never left the local checkout. A freeze coordinated over git is coordinated over exactly this known-broken channel.
- The 14:54 entry: ff-only pull of the human-facing checkout. Becomes load-bearing the moment execution leaves the host holding Zach's checkout — i.e. the moment this migration succeeds.
- The 14:13 entry: a moved project goes DARK in mandark's `scheduler status`, silently, printing stale timestamps that look like healthy last-run records. crt has been doing this for four days and nobody noticed. Under move-everything this becomes the DEFAULT state of every project. Recommendation (a) there — say "runs on dexter, last-run state not visible from here" instead of printing a stale record — should land BEFORE the migration, not after, or the migration's own progress becomes unobservable from the host running it.
Reader: these three are not side quests. Each one degrades specifically at the moment this migration succeeds, which is the worst possible time to discover them.
...
So: the runner runs, the gate holds legitimately, and dexter has dispatched nothing for three days.

A PREDICTION OF OURS WAS FALSIFIED, and you should have it because it changes what to build. realisateur predicted the 338 http_code=401 HOLDs were still live and were the cause. They are NOT -- http_code=200 throughout. The auth failure is gone. The gate is not broken; it is correctly pacing against a burn line at 23%/23%. This matters for design: the absence-surface must NOT be built to detect "the gate is malfunctioning", because in this instance nothing is malfunctioning. Everything is behaving exactly as specified and the outcome is still three silent days.

WHY THIS IS THE ACCEPTANCE TEST. dexter runs EXACTLY ONE participant (crt), and the burn-rate gate is GLOBAL rather than per-project. A persistently on-pace HOLD therefore means the entire host dispatches nothing -- a state that is invisible three ways at once: (1) dexter's own log says HOLD, which reads as normal because it IS normal; (2) mandark cannot see dexter's job state at all (silence-audit already FLAGs this class -- "reads job state under $HOME only, but 2 accounts dispatch"), and mandark's own record for crt still reads 2026-07-24T23:59:28; (3) steward-survey reads only schedule/_paced.conf and reports crt DARK, which is wrong in the opposite direction. Three sensors, three different wrong answers, zero of them saying "a weight-3 project has not run in three days."

CONCRETE ACCEPTANCE CRITERIA, offered for the step-3 surface. `scheduler` noargs should be able to say, without a human asking: (a) crt is enabled and last dispatched 3d ago -- MISSED, distinguished from DARK-and-expected; (b) dexter as a HOST has dispatched nothing in 3d while its runner is alive -- a host-level absence, not just a project-level one, which is the case a per-project view would miss entirely; (c) a project whose state lives on another host reads BLIND rather than being silently reported from stale local data -- today it is reported as a 07-24 timestamp with no indication that number is unreachable-and-stale rather than current. Note (c) is the one that would have prevented THIS session's wrong prediction.

A SECOND, SEPARABLE QUESTION, filed here because it is your engine and it may be the actual proximate cause -- realisateur did NOT diagnose it and is not asserting it: dexter's ~/.local/share/crt-nightly-batch/ contains a ZERO-BYTE sweep.lock dated 2026-07-25 20:10, sitting beside a sweep.log that stops at 20:37 the same evening. If that lock is held, normal. If it is STALE, it is a candidate proximate cause, and a stale lockfile silently suppressing dispatch is precisely the failure shape this week has been about. It cannot be answered from mandark (no key reaches dexter); it needs an fuser/flock probe there. Worth knowing whether the sweep loop has any stale-lock detection at all, or whether a lock outliving its process is indistinguishable from a live run.

ALSO WORTH A LOOK, same directory: expires_at was written 2026-07-25 01:14, which on the standard 7-day stamp puts the dead-man expiry around 2026-08-01. Per the switch's own documented semantics (confirmed live today in vkv-inventory, whose switch tripped 2026-07-27T20:51 and auto-disabled the job), the stamp is rewritten ONLY when the file is missing -- so if crt is still not dispatching by Aug 1, it acquires a second and independent reason not to run, and the first one will be masked by the second.

- **2026-07-28 16:11 (via `scheduler -i`):** MILESTONE PROPOSAL + SPRINT SEQUENCE (realisateur /ideate, 2026-07-28, Zach-directed and Zach-answered live). Filed through the front door rather than hand-edited: this is about scheduler's own engine and milestone, which realisateur does not own.

THE FINDING. Five projects independently filed the SAME finding this week, and it is one shape, not five: a component failed loud at the point of failure and silent everywhere else, and a human found it by accident. (1) scheduler q-756f82: ~10 unguarded notify-send calls in lib/sweep-loop-common.sh, incl. the dead-man switch trip at ~line 283, can block FOREVER -- the `2>/dev/null || true` idiom guards against FAILING, not against NEVER RETURNING. (2) dexter's paced runner logged 338 identical http_code=401 HOLDs over three days with zero dispatches, unnoticed. (3) chezz: Zach's three answers were committed by the ~30min sweep and sat UNPUSHED; chezz's nightly resets --hard to origin, so a third night of re-triage was averted only because a human happened to look. (4) gardien: dexter's timer failed 3 consecutive nights, all 3 mandark units have exit-1'd DAILY since 2026-07-24, noticed only when someone read a journal by hand. (5) ecosim/realisateur: silence-audit exists to mechanize exactly this and prints its own NOT WIRED banner.

Per realisateur/PRECIPITATION.md a cross-project cluster is answered by NAMING THE MISSING REGULATOR, not by promoting its members. The regulator: nothing witnesses whether the witnesses ran.

ZACH'S ANSWER (2026-07-28, interactive, stated): SCHEDULER OWNS IT -- it is the dispatcher. The reasoning he chose: scheduler is the only component that knows what was SUPPOSED to happen. Every other component can observe only what DID happen. A watcher without the schedule sees corpses; scheduler sees ABSENCES.

ACCRETION FREEZE -- NOT VIOLATED, and this is the crux. Zach also answered that the three-printable-views fold IS the failure fix, not a competing priority. So the absence-surface is NOT a fourth view: it is a PROPERTY OF THE FIRST ONE. `scheduler` noargs already prints now/next. "next" is a claim about the future, and the 338 HOLDs were a FALSIFIED "next" that no view was obligated to report. Making the first view accountable to its own prediction is not accretion -- it is the fold finally meaning something.

PROPOSED RESTATEMENT of the current milestone (scheduler's call to accept, amend or decline; realisateur is not declaring it): keep the three-printable-views bar exactly as written, and ADD to the `scheduler` noargs view the requirement that it be A COMPLETE AND FALSIFIABLE ACCOUNT OF THE SYSTEM -- if a dispatch was predicted and did not happen, that view says so; if it cannot know, it says BLIND, never silence. The BLIND vocabulary already exists in this repo (scheduler status learned it in 731e7b0) and in realisateur's closeout-lint worktree decision; it generalizes rather than being invented here.

SPRINT SEQUENCE -- Zach chose the full ordering, explicitly, over any single first step:
  1. SAFETY. Close q-756f82: either wrap the ~10 notify-send calls with `timeout 5` (cheap, no behaviour change when notify-send works) OR establish why the cron path is immune and WRITE THAT DOWN. Note q-756f82's own standing instruction, which realisateur endorses: do NOT close this by observing it has not hung in practice -- that is the argument that a silent failure is fine because nobody noticed. This is first because nothing else in the sprint is safe while the engine can silently stop, and every project is downstream of this file.
  2. MEASUREMENT. Inventory every fact scheduler already KNOWS but no view PRINTS -- for each, which of the 9 current views shows it (often: none). This is the measurement that says whether three views can actually carry everything or whether the fold is lossy. Cheap and offline.
  3. THE BAR. Build the absence-surface into `scheduler` noargs, DESIGNED AGAINST STEP 2'S EVIDENCE rather than against a guess. Acceptance witnesses, all real and all from this week, no fixtures needed: gardien's 3 missed dexter nights surface; the 338 401-HOLDs surface; a DARK-but-expected project reads as expected, not as missed; an unreadable cross-account job (aedile/vkv-inventory under svc-vaporwave) reads BLIND, not absent.
Zach's stated reason for taking the whole sequence: it is the only ordering where step 3 is designed against evidence.

ALSO DECIDED THIS SESSION, for context, not asks of scheduler: crt is to be RE-ENABLED ON DEXTER as part of its move (filed separately to crt); the dexter host policy reversal was applied to schedule/_paced.dexter.conf's header from mandark as a one-time Zach-directed COMMENT-ONLY edit (bccf9ce, pushed) because that header still asserted the superseded pin-by-need rule -- no participant line touched; and the FOCUS/QUESTIONS symlink layer is being retired in favour of GitHub issues rather than a transitional replacement, which is gardien's to design and is currently blocked on an invalid gh token on mandark (filed to gardien + a Zach blocker).

- **2026-07-28 15:08 (via `scheduler -i`):** THREE REGISTERED PROJECTS CANNOT BE PULLED BY A SECOND HOST -- their only remote is a filesystem path on mandark (2026-07-28, realisateur /ideate, found while making gardien deployable to dexter).

Surveyed every touched project's origin:
  gardien   /home/zach/git-remotes/gardien.git      <- mandark-only path
  crt       /home/zach/git-remotes/crt.git          <- mandark-only path
  senechal  /home/zach/git-remotes/senechal.git     <- mandark-only path
  chezz     git@github-chezz-deploy:hf7y/chezz.git
  scheduler git@github-scheduler-deploy:hf7y/scheduler.git

This is the same defect crt's own DEXTER-MOVE.md section 3 documents for potato, in its own words: "a local filesystem path that exists only on mandark. Potato therefore cannot fetch or pull, ever, and has silently drifted before." It survived MONTHS there. It is not a potato quirk -- it is the default shape for any project that was never pushed to a network remote, and it silently blocks that project from ever being migrated to, or deployed on, a second host. Given the ecosystem is actively moving work onto dexter, this is now a migration blocker sitting in three projects rather than a curiosity.

Worth noting WHY it is invisible: nothing fails. Development on mandark works perfectly against a local bare repo; the defect only manifests the first time a second host tries to clone, which is exactly the moment someone is busy doing something else. Same shape as the other findings today (338 silent 401s, three nights of failing timers) -- loud at the point of use, invisible until then.

RESOLVED for gardien this session: added git@github.com:hf7y/gardien.git as a second remote and pushed main; main/origin-main/github-main now all resolve to one sha. Deliberately added as a SECOND remote rather than replacing origin, because replacing it mid-session would have redirected in-flight focus-commit pushes -- but that leaves gardien with two remotes and a drift risk between them, which is not a stable end state.

TWO FOLLOW-UPS THIS CREATES, both scheduler's call:
1. schedule/gardien.conf still has REPO_URL="/home/zach/git-remotes/gardien.git". Dispatch clones from there, so nightly-batch and dexter would now be pulling from two different remotes that only agree as long as someone pushes to both. Either point REPO_URL at GitHub and retire the bare repo, or decide the bare stays canonical and GitHub is a mirror -- but the current both-and state should not persist.
2. crt and senechal have the same shape and no fix. crt's is already half-documented in its own DEXTER-MOVE.md; senechal's is not documented anywhere. Neither is urgent until something needs to pull them from another host -- but "we did not notice until we needed it" is precisely how potato drifted.

Reader: scheduler's own nightly-batch for item 1 (it owns the conf); item 2 is a heads-up for whenever crt/senechal migration comes up, not a task for scheduler to execute inside those repos.
