...
* Make the tasks for nightly jobs auditable and in one place, somewhere in scheduler but also symlinked or similar in the actual projects. Should be as simple as editing a text file to introduce a new idea
  ...
  - DONE (2026-07-18): bin/sync-crontab.sh --apply now symlinks each project's .claude/FOCUS.md into focus/<project>.md, mirroring questions/. Edit any project's nightly scope from one place. (Skips a project with no FOCUS.md rather than fabricating an empty one.)
