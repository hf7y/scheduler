...
# not ask. <count> is open milestones having an open issue A RUN COULD CLOSE
# -- one not carrying a blocked label (#587: an issue labelled `needs-human`
# held chezz's gate open on the authority of a question only Zach can
# answer). <next> is the title named by a `NEXT: <title>` line in an open
# milestone's description, or empty if none declares one (#582 -- GitHub
# milestones have no successor field, so this is the whole chain-declaration
# contract: written into the CURRENT milestone's description, by a human or a
# run, same as any other milestoned content per #575's ruling on authorship).
#
# Empty is BLIND and must NEVER read as count=0: that would stop all
# nineteen accounts on one outage, logged as deliberate. TWO calls now, not
# one: which issues are blocked is not on the milestones endpoint, only on
# the issues themselves (same shape as milestone_self_fed's own issues call,
# just below). Either call failing holds the whole probe BLIND.
milestone_gate_probe() {
  local slug="${1:?}" next blocked count
  next="$(timeout "${MILESTONE_GATE_TIMEOUT:-15}" gh api "repos/${slug}/milestones?state=open" \
    --jq '[.[].description // "" | capture("(?m)^NEXT:[ \t]*(?<t>.+)$")? | .t] | .[0] // ""' 2>/dev/null)" \
    || return 0

  blocked="$(milestone_blocked_labels)"
  case "$blocked" in
    *['"'\\\$\`]*) return 0 ;;  # malformed knob -> BLIND, same discipline tempo.sh applies to it
  esac
  ...
  count="$(timeout "${MILESTONE_GATE_TIMEOUT:-15}" gh api "repos/${slug}/issues?state=open&per_page=100" --paginate \
    --jq "[.[] | select(.milestone != null and .milestone.state == \"open\")
             | select( ( [(.labels // [])[].name] as \$l | (\"$blocked\"|split(\",\")) | any(. as \$b | \$l | index(\$b)) ) | not )
             | .milestone.number] | unique | length" 2>/dev/null)" \
    || return 0
