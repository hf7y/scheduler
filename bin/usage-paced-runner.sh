...
if [ -n "$REPO_ROOT" ] && [ -d "$REPO_ROOT/.git" ]; then
  # TRAP (#596): host mode runs this AS ROOT (it dispatches AS each account
  # via sudo below), but $REPO_ROOT is an unprivileged account's own
  # checkout. `git fetch` run bare here wrote root-owned remote-tracking
  # refs into it -- refs an unprivileged account can never again fetch over,
  # since it cannot rewrite root's lockfiles. Drop to the checkout's owner
  # first, same as every per-account dispatch in this file already does.
