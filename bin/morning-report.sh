...
# equivalent artifact (see the crt project's own printer/screen split,
# design discussion 2026-07-20): `scheduler` (the CLI) is the short/screen
# tier, DIGEST.md is the long/paper tier. Pass --no-digest to skip writing
# it (e.g. if this is being piped somewhere that doesn't want a side-effect
# file write).
...
if [ "$WRITE_DIGEST" -eq 1 ]; then
  {
    ...
    echo "     The 'read once, start to end' paper-equivalent tier;"
    echo "     'scheduler' (the CLI) is the short/screen tier. -->"
    echo
    printf '%s\n' "$body"
  } > "$DIGEST_FILE"
fi
