...
read fresh from the roster service every run.
  ...
  --arm     flip the roster row to 'live', POSTed to the service and
            confirmed by re-reading it (#686). Human-only (#291): refuses
  ...
  --now     dispatch this project ONCE, right now, as its own account.
            ...
            neither. Refuses if the account is not on this host (#432):
            the roster carries no host column left to hop by.
  ...
  --sprint <dur>
            ...
            --sprint 0 ends it early. Refuses off-host, same as --now.
  --sprint-status
...
# --- 3a. --arm/--park (#291, #686): POST the service, never converge. Runs
# before the wrong-host check -- a service write has no "wrong host" to be
# wrong about.
if [ "$MODE" = "--arm" ] || [ "$MODE" = "--park" ]; then
  NEW_STATE="live"
  ...
  write_roster_state "$PROJECT" "$NEW_STATE"; rc=$?
  [ "$rc" -eq 0 ] || exit "$rc"
  echo "${MODE#--}ed: '$PROJECT' -> $NEW_STATE, confirmed by re-reading the roster service"
  echo "next: run 'dose $PROJECT --apply' on the host that has its account to converge its crontab"
  exit 0
fi
