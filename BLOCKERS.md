...
- **2026-08-04 (vim-arcade `/cloture`): `closeout-lint`/`hygiene-lint`'s project-registry discovery reads `PROJECT_REPO_PATH` out of `schedule/*.conf` with `sed`, which never shell-expands `$HOME` — every conf that writes `PROJECT_REPO_PATH="$HOME/Documents/Projects/<name>"` (checked: `schedule/vim-arcade.conf` does) resolves to the literal, nonexistent path `$HOME/Documents/Projects/<name>`.** Reproduced directly: `closeout-lint` FLAGged `[missing-repo]` for all 8 recently-registered repos including vim-arcade and realisateur itself in the same run; `hygiene-lint vim-arcade` printed `(no git repo at that path -- skipped)` for a repo that in fact has 6 unpushed local commits at the moment it was run. Root cause, `realisateur/bin/closeout-lint.sh:146`: `sed -n 's/^PROJECT_REPO_PATH=["'\''\]\?\([^"'\''\]*\)["'\''\]\?[[:space:]]*$/\1/p' "$conf"` — this is a pure string capture, not a shell eval, so `$HOME` is never substituted; `hygiene-lint.sh` shares the same discovery loop per its own comment ("same loop as hygiene-lint.sh"). **Effect: because the missing-repo branch does `continue` past the age-gate, section A of `closeout-lint` currently cannot detect a dirty tree or unpushed commit in ANY registered project** — every closing session's "recently touched repos" signal has been silently empty since whichever conf started using `$HOME` instead of a literal path. Not fixed here — out of scope for a vim-arcade session and `/cloture`'s "report, route, do not build" posture — but this is a live false-all-clear in the shared instrument, not a one-off. Options as I see it: (a) fix the sed to only accept literal absolute paths and FLAG any conf using `$HOME`/other unexpanded vars as a conf-format defect; (b) switch discovery to actually sourcing each conf (`. "$conf"; echo "$PROJECT_REPO_PATH"`) so `$HOME` resolves the way every other consumer of these confs presumably expects; (c) something else. Whichever it is, re-run `closeout-lint` and `hygiene-lint` afterward and confirm the repo count of FLAGged `[missing-repo]` drops to the real number (0, as of this filing).
