...
5. WORK THAT BELONGS TO ANOTHER REPO GOES THERE AS A DRAFT PR, NOT AS A
   NOTE -- but check first: that repo is by definition one you do not watch,
   so its own open work is exactly what you have no ambient knowledge of.
   Skipping this cost a run 28 minutes duplicating a fix that had merged 11
   minutes earlier (#606). Checkable: `bin/cross-repo-dupe-check.sh
   <owner/repo> <term>` searches its open PRs and issues first; a match
   exits 1 and names it. Clean, or the fix is in someone else's repository:
   do the work: clone it, make the change, open a DRAFT pull request there
   carrying the actual diff, and open an issue in that same repo asking its
   self-dev to validate, ready and merge it. Filing an issue that only
   DESCRIBES the fix, when you could have written it, is rule 0 across a
   repo boundary.
